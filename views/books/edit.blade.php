...
@section('page_title', 'Edit Book Details')

@section('content')
<div class="mb-6">
    <a href="/books/{{ $book['id'] }}" class="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-200 transition">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
        Cancel and Return
    </a>
</div>

<form action="/books/{{ $book['id'] }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="space-y-6">
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col items-center">
            ...
            <!-- Cover Preview Box -->
            <div id="cover-preview-container" class="w-full max-w-[200px] aspect-[3/4] bg-slate-950 rounded-xl border-2 border-slate-800 flex flex-col items-center justify-center p-6 text-center text-slate-600 relative overflow-hidden group transition">
                @if($book['cover_image'])
                    <img id="cover-preview-img" src="{{ $book['cover_image'] }}" alt="Cover Preview" class="absolute inset-0 w-full h-full object-cover">
                @else
                    <svg id="cover-placeholder-svg" class="w-10 h-10 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    <p id="cover-placeholder-p" class="text-xs">No image selected</p>
    ...
    <div class="lg:col-span-2 space-y-6 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" id="total_pages" name="total_pages" required min="0" value="{{ $old['total_pages'] ?? $book['total_pages'] }}"
            ...
            <div>
                ...
                <input type="number" id="current_page" name="current_page" min="0" value="{{ $old['current_page'] ?? $book['current_page'] }}"
                    class="block w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition duration-200 text-sm">
            ...
            <div>
                ...
                <select id="reading_status" name="reading_status" required
                    class="block w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition duration-200 text-sm">
                    <option value="to_read" {{ ($old['reading_status'] ?? $book['reading_status']) === 'to_read' ? 'selected' : '' }}>To Read</option>
                    <option value="reading" {{ ($old['reading_status'] ?? $book['reading_status']) === 'reading' ? 'selected' : '' }}>Currently Reading</option>
                    <option value="completed" {{ ($old['reading_status'] ?? $book['reading_status']) === 'completed' ? 'selected' : '' }}>Completed</option>
                </select>
                        ...
                        @foreach($tags as $tag)
                            <label class="flex items-center gap-2 text-xs text-slate-300 cursor-pointer select-none">
                                <input type="checkbox" name="tags[]" value="{{ $tag['id'] }}" 
                                    {{ in_array($tag['id'], $old['tags'] ?? $bookTagIds) ? 'checked' : '' }}
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="description" name="description" rows="4"
                    class="block w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition duration-200 text-sm"
                    placeholder="Enter summary or physical book details...">{{ $old['description'] ?? $book['description'] }}</textarea>
...
<script>
    ...
    const placeholderSvg = document.getElementById('cover-placeholder-svg');
    const placeholderP = document.getElementById('cover-placeholder-p');
    ...
    coverInput.addEventListener('change', (e) => {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                ...
                if (placeholderP) placeholderP.classList.add('hidden');
